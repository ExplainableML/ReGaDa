...
class Evaluator:
    def __init__(self, dset):
        self.dset = dset
        pairs = [
            (dset.adverb2idx[adv.strip()], dset.action2idx[act])
            for adv, act in dset.pairs
        ]
        ...
        for _adv in dset.adverbs:
            mask = [
                1 if (_adv == adv or _adv == dset.antonyms[adv]) else 0
                for adv, act in dset.pairs
            ]
    ...
    def get_scores(
        ...
        if not use_no_act_gt:
            scores = {k: v.cpu() for k, v in scores.items()}
            scores = torch.stack(
                [scores[(adv, act)] for adv, act in self.dset.pairs], 1
            )
        action_gt = action_gt.cpu()

        action_gt_scores, action_mask = self.get_gt_action_scores(scores, action_gt)
        antonym_action_gt_scores = self.get_gt_action_antonym_scores(
            scores, action_gt, adverb_gt
        )
